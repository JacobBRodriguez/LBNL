...
class spyspark_client(object):
    ...
    def _parse_TS_data_json(self, res, result_type):
        ...
        try:
            TSdata["ts"] = pd.to_datetime(
                TSdata["ts"].str.replace(pat, repl))  # change into datetime type and apply regex
        except:
    ...
    ###############################################################################
    def request(self, query: str, result_format: str = "application/json",
                result_type: str = "meta"):  ## -> str: removed type returned, more flex!
    ...
    def _query_now(self, metadata):
        ...
        for item in metadata[
            'id']:  # for each meter id in metadata dataframe, query for timeseries data and append to main dataframe
    ...
    def get_ts_from_meta(self, metadata):
        ...
        for meter in metadata[
            'id']:  # for each meter id in metadata dataframe, query for timeseries data and append to main dataframe
            query = 'readAll(equipRef==@' + meter.split(' ')[0][
                                            2:] + ').hisRead(date(2010,01,01)..date(' + date + '), {limit: null})'
    ...
    def _find_percent_ok(self, frame):
        ...
        """
        percent_bad = frame['missing_perc'] + frame['zeroVal_perc'] + frame['flatline_perc'] + frame[
            'outliersStdv_perc']
    ...
    def run_analysis_from_meta(self, metadata, Sheets_url, save_each_meter_to_csv=False):
        ...
        credentials = ServiceAccountCredentials.from_json_keyfile_name("calendar_API_key.json",
                                                                       scope)  # Json comes from Google API
        ...
        for item in metadata['link']:  # For each link number in list
            ...
            if not math.isnan(float(item)):
                ...
                info_get = self.query(
                    'readAll(equipRef->link=="' + item + '").hisRead(date(' + date_time_start + ')..date(' + date_time_end + '), {limit: null})')
                time_dict = {}
                manual_read_value = {}
                skyspark_value = {}
                energy_column = ""
                ...
                for k, i in enumerate(months_list):
                    ...
                    try:
                        ...
                        ts_data = (float(ts_data.sum()[
                                             0])) / multiplier  # Multiplier = 1 for electricity meters and 10^5 for gas meters
                        for row in link_num_list:
                            if row[1] == link_float:

                                if pd.to_datetime(row[0]) == datetime.datetime.strptime(months_list[k + 1], '%m/%d/%Y'):
                                    time_dict[months_list[k + 1]] = ((float(row[4].replace(',', '')) - ts_data) / float(
                                        row[4].replace(',', ''))) * 100  # Percent different
