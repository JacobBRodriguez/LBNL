...
# Function to create list of clickable options to exclude meters from analysis
    def show_clickable_options(self):
        ...
        """ Function generates button widget for each option for point exclusion.
            Options include: Volume, Rate, Power, and Raw
        
        """
          
        list_of_options = ["volume","rate","power","raw"]
        self.volume_button = widgets.Checkbox(
                                    value=False,
                                    description="volume",
                                    disabled=False
                                    )
        self.rate_button = widgets.Checkbox(
                                    value=False,
                                    description="rate",
                                    disabled=False
                                    )
        self.power_button = widgets.Checkbox(
                                    value=False,
                                    description="power",
                                    disabled=False
                                    )
        self.raw_button = widgets.Checkbox(
                                    value=False,
                                    description="raw",
                                    disabled=False
                                    )
        display(self.volume_button)
        display(self.rate_button)
        display(self.power_button)
...
# Helper function to return list of dates that we will be running the analysis on for meter quality checks
    def get_dates_list(self, worksheet, month_range):
        ...
        month_range : List
            List containing start and end date strings to set date range for analysis.
...
# Helper function to build query string with appropriate return points specified
    def _construct_query(self, link_number, start_date, end_date):
        ...
        ----------
        link_number : String
            String representation of meter link number in Skyspark
        start_date : String
            String of start date for time frame grab with form mm/dd/YYYY
        end_date : String
            String of end date for time frame. Same form as start_date
            
        Returns
        -------
        query : String
            String representation of query used to retrieve data from Skyspark
            
        """
        ignored_points = []
        if self.volume_button.value:
            ignored_points.append("volume")
        if self.rate_button.value:
            ignored_points.append("rate")
        if self.power_button.value:
            ignored_points.append("power")
        if self.raw_button.value:
            ignored_points.append("raw")
        
        query = 'readAll(equipRef->link=="'+link_number+'"'
        for items in ignored_points:
            query = query +' and not '+items
            
        query = query + ').hisRead(date('+start_date+')..date('+end_date+'), {limit: null})'
        return query
    ...
    def run_analysis_from_meta(self, metadata, Sheets_url, month_range, save_each_meter_to_csv=False):
        ...
        for item in metadata['link']: # For each link number in list
            ...
            if not math.isnan(float(item)):
                ...
                single_point_query = self._construct_query(item, date_time_start, date_time_end)
                info_get = self.query(single_point_query)
                time_dict = {}
                manual_read_value = {}
                skyspark_value = {}
                energy_column = ""
